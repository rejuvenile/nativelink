...
use nativelink_util::common::{DigestInfo, VIOLATION_TYPE_MISSING, make_precondition_failure_any};
use nativelink_util::digest_hasher::DigestHasherFunc;
use prost::Message as _;

/// The wire `type_url` our `make_precondition_failure_any` stamps on the
/// `Any` detail. Asserted directly (rather than through the private
/// `action_messages::TypeUrl` trait) so the test stays decoupled from that
/// internal helper.
...
// Construct the error exactly as it reaches `to_execute_response` in
// production: the CAS returns a `NotFound` carrying a MISSING
// `PreconditionFailure` detail (`Error::not_found_with_detail` +
// `make_precondition_failure_any`), and the worker re-stamps the code to
// `FailedPrecondition` WITHOUT dropping the detail (see
// `local_worker.rs` `is_cas_blob_miss` -> `translated.code =
// Code::FailedPrecondition`). `to_execute_response` is then a pure
// passthrough, so the test builds the post-upgrade error directly.
fn make_missing_blob_error(digest: &DigestInfo) -> Error {
    let mut err = Error::not_found_with_detail(
        format!(
...
fn assert_missing_blob_status(status: &nativelink_proto::google::rpc::Status, digest: &DigestInfo) {
    ...
    assert_eq!(status.details[0].type_url, PRECONDITION_FAILURE_TYPE_URL);
    ...
    assert_eq!(pf.violations[0].r#type, VIOLATION_TYPE_MISSING);
    assert_eq!(
        pf.violations[0].subject,
        format!("blobs/{}/{}", digest.packed_hash(), digest.size_bytes()),
        "subject must follow REv2 `blobs/<hash>/<size>` format",
    );
    // Our `make_precondition_failure_any` leaves `description` empty by
    // design (the recovery signal is the subject digest, not prose).
    assert!(
        pf.violations[0].description.is_empty(),
        "make_precondition_failure_any leaves description empty",
    );
}
...
fn to_execute_response_does_not_misclassify_error_without_context() {
    // `to_execute_response` is a pure passthrough — it NEVER string-matches
    // the message. An error whose text mentions "Object … not found" but
    // carries no `PreconditionFailure` detail (the worker never ran the
    // `is_cas_blob_miss` upgrade) must surface without a detail.
    let action_result = action_result_with_error(Some(make_err!(
        Code::NotFound,
    ...
    assert!(
        status.details.is_empty(),
        "errors without a PreconditionFailure detail must not produce one",
    );
}
