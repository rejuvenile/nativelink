...
- **Path-mapping (CORRECTED 2026-07-17):** FL build runs `--experimental_output_paths=strip` + `supports-path-mapping`. Bazel's `StrippingPathMapper` **REPLACES the config mnemonic with the literal `cfg`** (`FIXED_CONFIG_SEGMENT`), it does NOT drop it → the path-mapped primary is `bazel-out/cfg/bin/…` (NOT `bazel-out/bin/…` and NOT `bazel-out/<mnemonic>/…`). **The allowlist prefix on all three sides (server `action_output_allowlist`, Bazel client, `rustc.bzl`) MUST be `bazel-out/cfg/bin/third_party/rust/apple_a14/`** or the carrier `primary_output` never matches → dead feature. Config-discrimination still holds via the `-<hash>` config-salt in the `.rlib` filename (not the dir); the `-incr` tree dir has no salt → when it's the bytewise-min output the key is CONFIG-SHARED → cross-config = cold-not-wrong. (Corrects the chunk-1 "config-discriminating" doc AND the earlier "no bazel-out/<config> segment" note.)
...
- **PROVISIONING DEPENDENCY — RESOLVED 2026-07-17 (Bazel team).** `/Volumes/CrowAgent/fl-incr-execroots` created on ALL 10 workers (worker-01 … worker-10) as a REAL dir (not symlink), `user:staff` 0755, reboot-durable via a RunAtLoad LaunchDaemon (`com.example.fl-incr-execroots`). **Hardlink topology RESOLVED (no contradiction — two machine types):** On the WORKERS, `/Volumes/CrowAgent` is a PLAIN DIRECTORY (not a mount); `/Volumes` lives on the Data volume, so it's the SAME volume/device as the worker CAS (`content_path=/Users/user/Work/nativelink/...`) → CAS→FIXED_PREFIX hardlinks cleanly (verified: `/Volumes`==`/Users` same device on all 10; live `ln` returns same inode; NO EXDEV). On the BUILD AGENTS, `/Volumes/CrowAgent` IS the real `disk5s1` volume and the bazel `output_base`/execroot also live on `disk5s1`, so hardlinks work there for a different reason (never hardlink from `/Users`). So on each machine type FIXED_PREFIX is co-located with that machine's input source → hardlinks on both, NO copy-fallback. **Implication for chunk 2a/2b: the §12 EXDEV probe MUST be a runtime `link()` test, NOT a hardcoded device constant — the worker device minor varies per host (16777230-34); on each `/Volumes`==`/Users` is what matters.** Caveat: if the worker CAS ever moves off `/Users`(Data), it'd cross-volume → the §12 probe falls back to copy (correct). **Cross-machine reuse PROVEN on the real M4 fleet:** seed built on worker-01 → byte-identical path on worker-02 → rustc incremental reuse fired (warm 0.04s vs cold 0.15s, ~4×). The FL-1383 premise holds on prod worker hardware.
