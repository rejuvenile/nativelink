...
- **PROVISIONING DEPENDENCY — RESOLVED 2026-07-17 (Bazel team).** `/Volumes/CrowAgent/fl-incr-execroots` created on ALL 10 workers (worker-01 … worker-10) as a REAL dir (not symlink), `user:staff` 0755, reboot-durable via a RunAtLoad LaunchDaemon (`com.example.fl-incr-execroots`). Reported: hardlinks CrowAgent→/Users (CAS) verified working, no EXDEV, on all 10 (DOUBLE-CHECK: this contradicts the earlier "CrowAgent=disk5s1 ≠ /Users=disk3s5 → EXDEV" topology — if genuinely different volumes, cross-volume hardlink should EXDEV; chunk 2a's §12 link-probe adapts at runtime either way, so not blocking, but confirm the volume topology so hardlink-vs-copy isn't a surprise). **Cross-machine reuse PROVEN on the real M4 fleet:** seed built on worker-01 → byte-identical path on worker-02 → rustc incremental reuse fired (warm 0.04s vs cold 0.15s, ~4×). The FL-1383 premise holds on prod worker hardware.
