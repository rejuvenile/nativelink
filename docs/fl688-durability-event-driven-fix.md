...
- **WEAK SPOT to BUILD/verify:** the `stale_pin` WARN tripwire the design leans on for "surface the bug, don't mask it" may NOT exist — the flow-trace grep found only `expire_stale_pins` (120s TTL, indefinite pins EXEMPT) and no "indefinite pin held too long without BIS" WARN. (CONTRADICTS the earlier live-observed 550-WARN/log signal → the code evolved to the v3 delta protocol; RECONFIRM whether the WARN exists.) If absent, BUILD it — it's the load-bearing bug-surfacing signal (a WARN/counter, NOT a pin-releasing sweep).
