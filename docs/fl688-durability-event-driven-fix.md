...
## DESIGN CADRE 2026-07-17 — auditor FIX-FIRST + pair-a FIXES-REQUIRED (distsys)/Needs-justification (red-team), CONVERGENT. NOT implementation-ready. `.claude/reviews/fl688-advertise-design/`
- **ARCHITECTURAL AXIS SIGNED OFF (distsys):** advertise-on-pin rides the EXISTING worker→worker_api `BlobsAvailable` wire; `worker_api` gets its CAS handle via `MAIN_SCHEDULER.simple.cas_store="cas_STORE"` (`prod-server.json5:247`, shared by-Arc in-process) so `has_durably`/`mark_stable` reach the slow tier. NO new cross-entry wire → the prior e546af44 cross-entry BLOCK is CLEARED; no fresh USER architectural authorization needed (no invariant broken, no wire added). advertise-on-pin (event-once) ≠ the Stage-A-removed PERIODIC AC-pin heartbeat (b505888b, different registry) — Chesterton's Fence respected.
- **F2 (HEADLINE, FIXES-REQUIRED — the crux): fire-once-at-PIN re-creates the original advertise-before-durable miss for the NOT-DURABLE-AT-PIN race.** The `BlobsAvailable` resend buffer drains on `BlobsAvailableAck` = a DELIVERY ack, NOT a durability ack — once receipt is acked, resending stops (~1 RTT of `has_durably` evaluations, not until-durable). Pre-mortem: a CI storm re-produces F2 outputs made durable by a CONCURRENT uploader's BatchUpdateBlobs-skip microseconds AFTER each worker pinned them (not-yet-durable-at-pin); advertise-on-pin fired once at `has_durably=None`, got a delivery-ack, stopped; D is durable but never BIS-acked → dark until reconnect → pins climb to `indefinite_pin_cap` → F2 uploads stall fleet-wide (the exact leak, via a window the 8-digest soak never drove). The design's robustness rests ENTIRELY on the unproven-universal "leak ⟹ already-durable-AT-PIN" necessary condition (`empirical-refutation-covers-only-soaked-interleavings`). **THE FIX: advertise when DURABILITY IS CONFIRMED, not at pin.** The reliable durability-confirmation event is the worker's deferred-UPLOAD COMPLETION: `Ok`(fresh write) → the server write-pusher already fired BIS; `AlreadyExists`(already durable) → ADVERTISE D → server `has_durably`=Some → `mark_stable`→BIS. This covers the already-durable-at-pin AND became-durable-during-upload cases, event-driven, fire-once, NO re-drive/sweeper. (Flow-trace nuance: the upload READ already advertises via `on_get` EXCEPT on the `cached_data` pre-read branch `running_actions_manager.rs:8142` — so the dark set = pre-read uploads; advertise-on-upload-completion covers it uniformly.) red-team alt: (a) bounded re-advertise-WHILE-PINNED [covers it but the operator called it a sweeper — it re-drives/masks a missed mark_stable]; (b) response-carried unpin [operator rejected — not connection-resilient]. **OPERATOR DECISION NEEDED: the trigger.**
- **F1 (FIXES-REQUIRED):** the fire-once dedup set is a new buffer on the F2-pin path → needs a measured cap + `// CAPPED AT N`/`// UNBOUNDED-OK` (also answers "prove bounded fan-out"). SPEC it.
- **F3 (MAJOR, convergent w/ auditor): the tripwire EXISTS** — `sweep_stale_indefinite_pins` (`moka:2438`, `nativelink::stale_pin_alert`, 30s, 4 tests) = the 550-WARN signal. Reference it; build NOTHING. VERIFY its target passes the prod log filter (else dark).
- **F5 (MINOR):** the synthesized pin/upload-advert's STAMP is load-bearing — `register_blobs_gated` is ts-gated; a `(0,0)`/frozen stamp can LOSE an LWW tie to a prior evict (`local_worker.rs:2413-2422`) → delta suppressed → silent no-op. Specify a stamp that OUT-RANKS any prior evict; test at the register/LWW seam.
- **V3 SPEC still BLOCKED (both):** the prior ACTION-5-refusion + monotonic-locality spec BLOCK is NOT cleared by a flow-trace — re-do `MarkStableAsyncDurabilityV3` (model the advertise trigger, the delivery-vs-durability ack, the LWW stamp, non-monotonic locality) BEFORE implementation. "flow-trace VALIDATED" ≠ formal validation.
- **F2 REFUTED — DESIGN COMPLETE (invariant-prover re-verify + TLA+ red/green, 2026-07-17, `specs/MarkStableAdvertiseOnPinV3.tla`).** The invariant "a fresh durable landing of D fires a pusher→GLOBAL BIS that reaches+unpins EVERY holder of an indefinite pin for D, even one that never advertised" is CONFIRMED end-to-end (all falsifiers absent): FSS fresh-write `Ok` arm pushes unconditionally (`fast_slow_store.rs:6519`/`:6823`/`:3529`, no has_durably/locality guard); drain broadcasts globally (`nativelink.rs:1300-1443`); the broadcast is a pure fan-out to ALL workers (`api_worker_scheduler.rs:9268` `inner.workers.iter()`, no filter); the worker unpins on its OWN pin set unconditionally (`local_worker.rs:3238-3263`→`moka:1847` `unpin_key`, the `record_bis_unpin` gate touches only the metric). No first-durable-without-pusher path (the already-durable ECS/BatchUpdateBlobs skips fire only when a PRIOR fresh write already pushed+BIS'd). **⇒ advertise-on-pin need ONLY cover the durable-BEFORE-pin case; the durable-AFTER-pin case is FREE (global BIS reaches the holder regardless).** TLC: Bugged (no advertise-on-pin) VIOLATES `DurableHeldReleasedAtQuiescence` (the before-pin leak); Fixed passes (4225 states); Witness proves the after-pin release is advertise-INDEPENDENT; FixedNonVacuity proves the advertise path fires; `StableFeedIsDurable` holds in both (liveness-only, ≥2-replica intact, no TTL). **The operator's advertise-on-pin design is COMPLETE on the durability-event seam.**
- **IMPLEMENTATION SCOPE (proceeding):** (1) advertise-on-pin — feed the pinned F2-output digest into the `BlobChangeTracker`/`digest_infos` channel at the pin site (`running_actions_manager.rs:7803`, which does NOT hold the tracker → thread a handle); (2) F1 — a `// CAPPED AT N` fire-once dedup set so a re-pinned digest doesn't re-advertise every window; (3) F5 — the injected advert's STAMP must out-rank a prior evict so `register_blobs_gated`'s LWW doesn't suppress it (`local_worker.rs:2413-2422`); (4) F3 — REFERENCE the existing `sweep_stale_indefinite_pins` tripwire (`moka:2438`, `nativelink::stale_pin_alert`), build no duplicate + verify its target passes the prod log filter. Commit `MarkStableAdvertiseOnPinV3` as the spec. Final cadre on the diff after.
