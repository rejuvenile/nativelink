...
**Status (2026-05-08)**: Phase 1 (push-based invalidation) IMPLEMENTED on `redis-eviction-notify` plus 100-fixup for #100 (AC store eviction callbacks). **Defensive infrastructure for Redis (Valkey) keyspace eviction notifications. Currently dormant in production (Valkey on buildcache has had zero evictions in this server's lifetime — `evicted_keys: 0`, `expired_keys: 0`, `used_memory: 548 MB / 251 GB`); deployed for forward-compat against future workloads stressing the eviction path.** When keys ARE evicted under `maxmemory-policy=allkeys-lru`, the dispatcher fires `ItemCallback::callback` for every registered consumer (today: `ExistenceCacheStore`, which removes the corresponding entry). Phase 2 (SCAN reconciler on reconnect) and Phase 3 (overflow-triggers-reconcile) remain DEFERRED — Phase 1 ships without Phase 3 because the chosen unbounded mpsc for callback registration carries no event payload (registrations are extremely rare; it's the `subscriber_channel` that carries events, and that's already unbounded by the redis crate). On overflow of the inflight `JoinSet`, the dispatcher applies cooperative backpressure via `join_next().await` rather than dropping events.

**100-fixup (2026-05-08)** addresses the 6 BLOCKER findings from code-reviewer / perf-optimizer / security-reviewer / red-team / distributed-systems-reviewer:
- **BLOCKER 1 (silent init failure)**: eager init at construction in `RedisStore::new_standard` propagates CONFIG GET / SET / PSUBSCRIBE failures as `Err`; the server fails to start instead of silently running with a stale-positive cache. `register_item_callback` is now a synchronous fast-path-only operation: success or `Err` (no background spawn that swallows failures).
- **BLOCKER 2 (consume-before-validate `take()`)**: `subscriber_channel.lock().take()` deferred to LAST step of the eager init so retries do not hit a misleading "already consumed by SchedulerSubscriptionManager" error.
- **BLOCKER 3 (`StoreKey::Str` over-action)**: `parse_keyspace_payload` filters non-digest payloads (returns `None`) so admin `DEL cas:debug-foo` and foreign scheduler-index keys do not propagate into ECS or other ItemCallback consumers. The Str fallback was a perf+log-leak hazard, not a panic risk (`StoreKey::into_digest()` blake3-hashes the bytes), but a guaranteed no-op cache miss is still pure waste.
- **BLOCKER 4 (CONFIG SET non-persistent)**: `StandardRedisManager::reconnect_with` re-issues `CONFIG SET notify-keyspace-events <flags>` on every subscriber-slot reconnect, closing the silent-degradation window after a Valkey restart that reverts the in-memory CONFIG. **Operator should ALSO persist `notify-keyspace-events Eegx` in `valkey.conf`** for resiliency against operator restarts of NativeLink itself (between which the runtime CONFIG state is owned by the previous client).
- **BLOCKER 5 (shared subscriber_channel)**: `set_spec_defaults` rejects the `enable_keyspace_notifications=true` + `experimental_pub_sub_channel.is_some()` collision at construction time with `Code::FailedPrecondition`. Fan-out task (per design §1.1) remains future work — required only when both consumers must coexist on a single `RedisStore`.
- **BLOCKER 6 (CI gating)**: `redis-integration-tests` feature wired into `.github/workflows/native-cargo.yaml` with valkey-server install step (Linux runners only). Default `cargo test` on dev boxes still skips the bin (no silent skip — feature-gated by `#![cfg(...)]`, so the test bin is excluded entirely).

Hardening additions (security-reviewer MEDIUMs):
- `keyspace_notifications_db: i64` → `u8` (Redis db indices are 0–255 in practice, ≤16 by default), preventing negatives and over-wide values from silently producing nonsense PSUBSCRIBE patterns.
- `set_spec_defaults` cross-checks `keyspace_notifications_db` against the URL-embedded db (`redis://server/3` vs `keyspace_notifications_db: 0` mismatch is now `Err` instead of silent listen-on-wrong-db).
- Empty `key_prefix` with `enable_keyspace_notifications=true` now `warn!`s at construction (foreign-tenant DELs would otherwise dispatch into ItemCallback consumers — acceptable on dedicated Redis, footgun otherwise).
