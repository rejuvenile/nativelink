...
**Why not existing `buf_channel`?** The existing `DropCloserWriteHalf`/`DropCloserReadHalf` in `/src/nativelink/nativelink-util/src/buf_channel.rs` is a 1:1 channel (single producer, single consumer). It uses `mpsc::channel` internally. The streaming blob needs 1:N fan-out. The existing buf_channel should remain as-is for point-to-point streaming; `StreamingBlob` is a new primitive for the concurrent-read case.
