...
pub fn download_to_directory<'a>(
    ...
    async move {
        ...
        let fetcher_fut = async {
            ...
            // Post-fetch integrity verification: read back every fetched blob
            // from the fast store and verify its blake3 hash matches the
            // expected digest. This catches corruption introduced during
            // download (GrpcStore), storage (FilesystemStore), or transport.
            let verify_start = std::time::Instant::now();
            let fetched_digests: Vec<DigestInfo> =
                fetched_set_ref.lock().unwrap().iter().copied().collect();
            let mut verify_ok: usize = 0;
            let mut verify_fail: usize = 0;
            for digest in &fetched_digests {
                if is_zero_digest(*digest) {
                    verify_ok += 1;
                    continue;
                }
                let (mut tx, mut rx) = make_buf_channel_pair();
                let fast_store = Pin::new(cas_store.fast_store());
                let key: StoreKey<'_> = (*digest).into();
                let get_fut = fast_store.get_part(
                    key,
                    &mut tx,
                    0,
                    None,
                ...
                let hash_fut = async {
                    ...
                    Ok::<(DigestInfo, u64), Error>((computed, total))
                };
                let (get_res, hash_res) = tokio::join!(get_fut, hash_fut);
                match (get_res, hash_res) {
                    (Ok(()), Ok((computed, total))) => {
                        if computed.packed_hash() != digest.packed_hash()
                            || total != digest.size_bytes()
                        {
                            verify_fail += 1;
                            error!(
                                expected_hash = %digest.packed_hash(),
                                expected_size = digest.size_bytes(),
                                actual_hash = %computed.packed_hash(),
                                actual_size = total,
                                "INTEGRITY FAILURE: fetched blob hash mismatch after \
                                 download to fast store — data corruption detected",
                            );
                        } else {
                            verify_ok += 1;
                        }
                    ...
                    (get_err, hash_err) => {
                        ...
                        error!(
                            digest = ?digest,
                            get_err = ?get_err.err(),
                            hash_err = ?hash_err.err(),
                            "INTEGRITY FAILURE: could not verify fetched blob",
                        );
                    }
                }
            }
            ...
            if verify_fail > 0 {
                error!(
                    ...
                    verify_fail,
                    verify_ms,
                    "post-fetch integrity check: FAILURES DETECTED",
                );
            } else {
                info!(
                    verify_ok,
                    verify_ms,
                    "post-fetch integrity check: all blobs verified",
                );
            }
        };
