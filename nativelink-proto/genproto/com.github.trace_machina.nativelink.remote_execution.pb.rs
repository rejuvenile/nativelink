...
/// / Returned ONCE on success. See worker_api.proto for the full schema.
#[derive(Clone, PartialEq, ::prost::Message)]
...
/// #212 v4.5: NativeLink-owned CAS-side extensions service. Hosts the
/// `WriteChunked` RPC so it can be registered on the same listener as
/// the upstream `cas` / `bytestream` services and reached via the
/// worker's outbound CAS-endpoint channel. See
/// `cas_extensions.proto` for the schema rationale.
///
/// Generated client implementations.
pub mod cas_extensions_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        /// / CAS blob. Returns committed_digest + committed_size on
        /// / success. See WriteChunk for per-chunk schema.
...
pub mod cas_extensions_server {
    ...
    pub trait CasExtensions: std::marker::Send + std::marker::Sync + 'static {
        ...
        /// / wiring still compiles + reports a clean error to clients
        /// / that try to use the path before it is enabled.
