...
pub(crate) async fn write_chunk_at_offset(
    ...
    let result =
        tokio::task::spawn_blocking(move || -> Result<(u64, u64, Instant), std::io::Error> {
            ...
            #[cfg(target_family = "unix")]
            {
                ...
                // #449b: capture closure-completion timestamp as the LAST
                // statement before return. Outside the .await, the elapsed
                // since this Instant measures the gap between
                // closure-return and the caller's task being re-polled
                // (the JoinHandle resume cost). Discriminates spawn-pool
                // dispatch contention (Hypothesis B) from upstream worker
                // starvation (caller too busy to re-poll the JoinHandle).
                Ok((dispatch_us, pwrite_us, Instant::now()))
            }
    ...
    let (dispatch_us, pwrite_us, closure_complete_at) = blocking_result.map_err(|io_err| {
        make_err!(
            Code::Internal,
            "chunked pwrite at offset {chunk_offset} ({len} bytes) failed: {io_err:?}"
        )
    })?;
    // #449b: closure-to-resume gap = wall-clock between closure return
    // and caller resume. High when blocking-pool dispatch is fast but
    // the caller's task is starved (post-investigation 2026-05-12: this
    // is the missing measurement that discriminates pool-internal
    // contention from upstream worker starvation per
    // `.claude/audits/blocking-pool-saturation-investigation-20260512.md`).
    let closure_to_resume_us = closure_complete_at.elapsed().as_micros() as u64;

    // #449 inline split: emit a decomposition warn when the sum of the
    // four sub-stages exceeds the same 50 ms threshold the driver's
    ...
    //
    // Silence-is-diagnostic: `total_inner_us` measures mutex + dispatch
    // + pwrite + closure-to-resume, which excludes three spans the
    // driver's `back_edge_ms` includes: `open_or_create_partial.await`
    // at `:458`, `try_clone()` at `:495`, and `drop(file_guard)` at
    // `:563`. If the driver warn fires WITHOUT this decomposition warn,
    // the cost lives in one of those three — that absence IS the
    // operator-actionable signal pointing at FS overhead, dup syscall
    // contention, or mutex teardown.
    //
    // Cost: 4 extra `Instant::now()` calls per chunk (~40 ns) + four
