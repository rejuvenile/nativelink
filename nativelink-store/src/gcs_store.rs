    ...
    NowFn: Fn() -> I + Send + Sync + Unpin + 'static,
{
    ...
    /// GcsStore is a leaf — `mark_stable` is a no-op (GCS owns its own
    ...
    /// GcsStore is remote-disk-backed: a sustained latency hiccup at the
    /// GCS endpoint (throttling, regional brownout, network saturation)
    ...
    /// when wrapping a GcsStore. (Path C extension, cascade-bundle,
    /// 2026-05-09.)
