...
use crate::chunked_signal::{encode_backpressure_signal_any, error_has_backpressure_signal};
...
/// Bounded concurrency for the shutdown drain in
/// [`FastSlowStore::flush_fast_to_slow_at_shutdown`]. #210: the prior
/// implementation processed the fast-tier snapshot strictly serially
/// (`slow.has` RTT, then `slow.update_oneshot` RTT, one blob at a time). At
/// the 2026-06-23 17:38 restart this enumerated 1,418,996 SMALL_CAS_CACHED
/// keys and burned the entire 30 s budget on ~591 k serial Redis `has`
/// round-trips, flushing ZERO blobs (`flushed=0 deadline_exceeded=827204`)
/// before exit lost them. Draining K-at-a-time turns N serial RTTs into N/K,
/// so an unbounded drain of a large fast tier finishes in minutes, not the
/// ~tens-of-minutes a strictly-serial drain would take.
///
/// Capped at 64: each in-flight slot holds at most one blob's bytes
/// (`get_part_unchunked` materializes the blob) plus its `has`/`update`
/// futures. The fast tier here is a MemoryStore, so the bytes are already
/// resident — the cap bounds only the transient duplicate copies handed to
/// the slow tier, not new allocation pressure of consequence. 64 saturates a
/// Redis/Filesystem slow tier's pipeline without flooding it.
const SHUTDOWN_FLUSH_CONCURRENCY: usize = 64;

/// Emit an `info!` progress line every this many processed blobs during the
/// shutdown drain so an operator watching `journalctl` through a long
/// unbounded flush sees forward motion (flushed/skipped/remaining) and can
/// distinguish "patiently draining 800 k blobs" from "wedged on a dead slow
/// tier". Chosen so a multi-hundred-k drain logs on the order of tens of
/// lines, not one-per-blob.
const SHUTDOWN_FLUSH_PROGRESS_EVERY: usize = 50_000;
...
impl FastSlowStore {
    ...
    /// #334 Fix B: aggregate bytes pinned in `in_flight_slow_writes`,
    /// observed atomically via the lock-free counter (no map walk).
    /// Operator + test visibility into the slow-write pin watermark.
    #[must_use]
    pub fn in_flight_slow_write_bytes(&self) -> u64 {
    ...
    /// `in_flight_slow_writes` insert site. Returns `Ok(())` when the
    /// insert is permitted (cap is zero OR aggregate byte total +
    /// `incoming_bytes` would not exceed the cap); returns
    /// `Err(ResourceExhausted+BackpressureSignal::SlowWritesAtCapacity)`
    /// otherwise.
    ///
    /// The check is a SNAPSHOT of `in_flight_slow_writes_bytes` and is
    /// eventually-consistent under heavy concurrent writers — answer
    /// can be stale by one in-flight insert's worth of work. That is
    ...
    fn check_slow_writes_capacity_gate(&self, incoming_bytes: u64) -> Result<(), Error> {
        ...
        let current = self.in_flight_slow_writes_bytes.load(Ordering::Relaxed);
        if current.saturating_add(incoming_bytes) <= cap {
            return Ok(());
        }
        debug!(
            ...
            incoming_bytes,
            cap_bytes = cap,
            "FastSlowStore: emitting BackpressureSignal::SlowWritesAtCapacity \
             (in-flight slow-write byte cap would be exceeded)",
        );
        let detail = encode_backpressure_signal_any(
            backpressure_signal::Reason::SlowWritesAtCapacity,
            SLOW_WRITES_AT_CAPACITY_RETRY_MS,
        );
        Err(Error::resource_exhausted_backpressure(
            format!(
                "FastSlowStore in-flight slow-write byte cap exceeded: \
                 current {current} bytes + incoming {incoming_bytes} bytes > \
                 cap {cap} bytes. Retry after ~{SLOW_WRITES_AT_CAPACITY_RETRY_MS}ms."
            ),
            detail,
        ))
    }
    ...
    async fn update_via_chunked_dispatcher(
        ...
        // #334 Fix A (chunked-path sibling): same rationale as the
        // small-blob site (see `update`'s body). Latent today
        // (chunked-path admission rejections come from the
        // dispatcher's per-blob mpsc / global PinBudget, NOT
        // MemoryStore), but the contract is identical and a future
            ...
            let Err(err) = fast_res else {
                unreachable!(
                    "fast_res_carries_typed_backpressure predicate gates on \
                     fast_res.is_err(); fast_res must be Err here"
                );
            };
            error!(
                ?key,
    ...
    pub async fn flush_fast_to_slow_at_shutdown(&self) -> usize {
        ...
        // `buffer_unordered`'s higher-ranked `FnMut` bound ("implementation of
        ...
        let drain = futures::stream::iter(0..snapshot_count)
            .map(|idx| {
                let flushed = &flushed;
                let skipped_already = &skipped_already;
                let errored = &errored;
                ...
                async move {
                    ...
                    if n % SHUTDOWN_FLUSH_PROGRESS_EVERY as u64 == 0 {
                        ...
                        // unbounded shutdown drain sees motion, not a wedge.
                        info!(
                            snapshot_count,
                            flushed = flushed.load(Ordering::Relaxed),
                            skipped_already = skipped_already.load(Ordering::Relaxed),
                            errored = errored.load(Ordering::Relaxed),
                            remaining = snapshot_count as u64 - n,
                            elapsed_ms = u64::try_from(started.elapsed().as_millis())
                                .unwrap_or(u64::MAX),
                            "FastSlowStore::flush_fast_to_slow_at_shutdown: draining"
                        );
                    }
                    ...
                    // materializes the full blob — for MemoryStore this is a
                    // near-zero-copy fetch from the BytesWrapper chain. We do
                    // NOT stream via a buf_channel: the slow stores here are
                    // Redis or FilesystemStore and `update_oneshot` is the
                    // cheaper path when the data is already in memory.
                    let data = match self
                        .fast_store
...
impl StoreDriver for FastSlowStore {
    ...
    async fn update(
        ...
        // #334 Fix A: when `fast_res` carries a typed
        // `BackpressureSignal`, surface it to the caller BEFORE
        // checking `data_res`. Without this gate, a fast-tier
        // early-reject (e.g. `MemoryStore::check_backpressure_gate`
        // emitting `MemoryStoreAtCapacity`) would drop `fast_rx`,
        // causing the data-stream future's `(*fast_guard).send(...)`
        // to fail with channel-closed; the closure wraps that as
        // generic `Code::Internal "Failed to send message to
        // fast_store ..."`, and the match below would return the
        // Internal first — masking the typed signal. Bazel does NOT
        // honor backoff hints on Internal, so the cascade observed
        // 2026-05-08 (#334: 67 GB RSS / 1h15m, 2743 errors/min) was
        // self-perpetuating until the cap was lifted.
        //
        // Strict heuristic (option (a) in the task brief): require
        // BOTH `Code::ResourceExhausted` AND a `BackpressureSignal`
        // detail. A bare `ResourceExhausted` from a future fast-tier
        // store kind (e.g. an SSD ExistenceCache returning disk-full)
        ...
        if fast_res_carries_typed_backpressure {
            ...
            error!(
                ...
                ?err,
                "FastSlowStore::update: fast store rejected with typed \
                 BackpressureSignal — preserving across data-stream \
                 channel-closed wrapper (#334 Fix A)",
            );
            return Err(err);
        }
        ...
        // BEFORE the spawn. The pin holds `data` (the entire blob's
        // chunks) until the background slow-write terminates; under
        // slow-tier wedge that's unbounded growth without a cap. The
        // cap-check is conservative — uses the snapshot before the
        ...
        tokio::spawn(async move {
            ...
            // has already observed the terminal state.
            {
                ...
                // #334 Fix B: decrement counter ATOMICALLY under the
                // map mutex so the (counter, map) pair stays
                // monotonically consistent with the insert ordering.
                // Subtract the actual removed payload's byte total
                // (NOT `bytes_sent` from the captured closure scope)
                // so a future change that drops the entry under a
    ...
    async fn get_part(
        ...
        // #515 Phase 0 H1 hypothesis empirically refuted (audit:
        // `.claude/audits/515-phase02-empirical-refutation-\
        // 2026-05-17.md`). The original hypothesis was that the
        // producer could advance `earliest_chunk_idx` between the
        // pre-check load at `:6491` and `StreamingBlobReader::new`
        // below, leaving the reader CONSTRUCTED at cursor > 0 and
        // breaking the splice arithmetic. Production sampling of
        // reader construction (`streaming_blob.rs:642`) shows
        // `starting_chunk_idx = 0` for 100% of samples, and 585
        // splice firings post Phase 0.2 deploy produced ZERO DataLoss
        // events. The frankenstein-bytes mechanism is architecturally
        // invalid: readers always start at cursor=0. Demoted WARN →
        // INFO; this remains as a benign observation that a pre/post
        // mismatch occurred (expected when the producer concurrently
        // advances), useful for verifying the construction-time
        // invariant holds going forward.
            ...
            && post_construction_chunk_idx > 0
        {
            ...
            info!(
                ...
                offset,
                ?length,
                "#515 cursor_chunk_idx pre/post differ at reader_new \
                 (rare; expected when producer concurrently advances; \
                 benign per Phase 0.2 verification): \
                 pre={pre_check_earliest_chunk_idx} \
                 post={post_construction_chunk_idx}"
            );
        }
        let mut pos = 0u64;
                    ...
                    if is_populator_caller {
                        ...
                        if is_sliding_window_eviction {
                            ...
                            info!(
                                ...
                                ?new_length,
                                starting_chunk_idx,
                                "#515 splice firing (benign): reader fell behind \
                                 sliding window at cursor_chunk_idx={starting_chunk_idx}, \
                                 resuming from slow tier at new_offset={new_offset}"
                            );
                            self.metrics
                                .streaming_buffer_reader_fallback_to_direct_total
                                .fetch_add(1, Ordering::Relaxed);
