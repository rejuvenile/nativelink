...
impl StoreDriver for CacheMetricsStore {
    ...
    fn register_item_callback(
        ...
        self.backend.register_item_callback(callback)
    }

    // CacheMetricsStore is a transparent single-inner metrics wrapper: the
    // BIS / pin / durable chain flows through unchanged to `backend` (same
    // shape as ExistenceCacheStore's `Inner` delegations).
    fn stable_delegation(&self) -> StableDigestDelegation<'_> {
        StableDigestDelegation::Inner(self.backend.as_store_driver())
    }

    fn pin_delegation(&self) -> PinDelegation<'_> {
        PinDelegation::Inner(self.backend.as_store_driver())
    }

    fn mark_stable_delegation(&self) -> MarkStableDelegation<'_> {
        MarkStableDelegation::Inner(self.backend.as_store_driver())
    }

    fn durable_delegation(&self) -> DurableDelegation<'_> {
        DurableDelegation::Inner(self.backend.as_store_driver())
    }
}
