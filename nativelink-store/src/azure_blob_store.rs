    ...
    NowFn: Fn() -> I + Send + Sync + Unpin + 'static,
{
    ...
    /// AzureBlobStore is a leaf — `mark_stable` is a no-op (Azure owns
    /// object lifecycle; no BIS participation from this layer). (Task #157.)
    fn mark_stable_delegation(&self) -> MarkStableDelegation<'_> {
        MarkStableDelegation::Leaf
    }

    /// AzureBlobStore is remote-disk-backed: a sustained latency hiccup
    /// at the Azure Blob endpoint (throttling, regional brownout,
    /// network saturation) lets an unbounded in-flight write buffer pin
    /// one chunk per concurrent stream until OOM. Compositions like
    /// `FastSlowStore` that buffer slow-tier writes MUST carry an
    /// explicit non-zero cap when wrapping an AzureBlobStore. (Path C
    /// extension, cascade-bundle, 2026-05-09.)
