...
// NOTE (merge v1.6.1): the upstream-superseded `RemoveItemCallback` /
// `register_remove_callback` mechanism was DELETED in favor of the unified
// `ItemCallback` (insert/get/remove/pin-expired). The former
// `oversized_skip_fires_remove_callbacks` test exercised that deleted API on
// the old skip-on-oversize path; the merged `MemoryStore` early-rejects an
// oversized write with `MemoryStoreAtCapacity` (ResourceExhausted) instead of
// skipping, so the test's premise no longer exists. Dropped per merge policy
// (test of deliberately-removed behavior). Early-reject coverage lives in the
// `update_rejects_upfront_*` tests below.
