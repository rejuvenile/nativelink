...
/// `insert_local_ac_pin` records an AC entry in `dispatched_mirror_pins`
/// WITHOUT touching `mirror_blobs` (the data lives in the FSS fast tier,
/// not in the mirror map). The snapshot exposes the (store_id, digest)
/// tuple sorted by store_id ASCII so the worker's BlobsAvailable
/// `pinned_mirror_entries` field carries it to the server.
///
/// Mutation step: comment out
/// `self.dispatched_mirror_pins.lock().insert((key, digest), ());`
/// in `insert_local_ac_pin`. The "snap MUST hold ... entry" assertion
/// red-fails with "advertise contract violated — AC pin missing from
/// dispatched_mirror_pins".
#[nativelink_test]
async fn insert_local_ac_pin_advertises_without_mirror_blobs() -> Result<(), Error> {
    let fast_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let slow_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let fast_slow_store = FastSlowStore::new(
        &FastSlowSpec {
    ...
    // Empty initially — no AC pins, no mirror blobs.
    assert!(fast_slow_store.dispatched_mirror_pin_snapshot().is_empty());
    assert_eq!(fast_slow_store.mirror_blob_count(), 0);
    assert_eq!(fast_slow_store.mirror_blobs_used_bytes(), 0);

    // AC entries are tiny proto blobs — use a 92-byte synthetic digest
    // size that matches a typical ActionResult. The actual byte cap is
    // not exercised by `insert_local_ac_pin` (no data stored).
    let d = DigestInfo::try_new(VALID_HASH, 92).unwrap();
    fast_slow_store.insert_local_ac_pin("AC_MAIN_STORE", d);

    // The pin shows up in the snapshot.
    let snap = fast_slow_store.dispatched_mirror_pin_snapshot();
    assert_eq!(
        snap.len(),
        1,
        "snap MUST hold the AC pin entry inserted via insert_local_ac_pin — \
         advertise contract violated — AC pin missing from dispatched_mirror_pins; \
         got {snap:?}"
    );
    assert_eq!(snap[0].0.as_ref(), "AC_MAIN_STORE");
    assert_eq!(snap[0].1, d);

    // Critically: NO bytes were stored in mirror_blobs (AC data lives
    // in the FSS fast tier, not the mirror map). This guards against a
    // regression where someone mistakenly routes AC writes through
    // insert_mirror_blob and double-stores the bytes (memory waste +
    // capacity-cap interaction with CAS mirroring).
    assert_eq!(
        fast_slow_store.mirror_blob_count(),
        0,
        "AC pin MUST NOT populate mirror_blobs — bytes already in \
         FSS fast tier; double-storing wastes RAM and contends with \
         the CAS mirror-blob byte cap. mirror_blob_count = {}",
        fast_slow_store.mirror_blob_count()
    );
    assert_eq!(
        fast_slow_store.mirror_blobs_used_bytes(),
        0,
        "AC pin MUST NOT increment mirror_blobs_total_bytes \
         (otherwise it would erode the CAS mirror cap budget); \
         used_bytes = {}",
        fast_slow_store.mirror_blobs_used_bytes()
    );

    // remove_local_ac_pins drops the entry on BIS ack.
    fast_slow_store.remove_local_ac_pins(&[d]);
    let snap2 = fast_slow_store.dispatched_mirror_pin_snapshot();
    assert!(
        snap2.is_empty(),
        "remove_local_ac_pins MUST drop the matching entry from \
         dispatched_mirror_pins so the next BlobsAvailable tick \
         doesn't re-advertise an already-acked pin; got {snap2:?}"
    );

    Ok(())
}

/// `insert_local_ac_pin` MUST notify `mirror_changes_notify` so the
/// worker's BlobsAvailable loop wakes promptly (vs waiting for the
/// 60s backstop tick) when an AC entry is recorded during the
/// slow-write window.
///
/// Mutation step: comment out
/// `self.mirror_changes_notify.notify_one();` at the end of
/// `insert_local_ac_pin`. The `wait` future never completes and the
/// timeout fires with the bespoke message below.
#[nativelink_test]
async fn insert_local_ac_pin_wakes_blobs_available_loop() -> Result<(), Error> {
    use core::time::Duration;
    ...
    let notify = fast_slow_store.mirror_changes_notify();
    // Arm a notified future BEFORE the insert so the permit is consumed
    // by the insert's notify_one.
    ...
    tokio::time::timeout(Duration::from_secs(3), &mut waiter)
        .await
        .expect(
            "insert_local_ac_pin MUST notify mirror_changes_notify so \
             the BlobsAvailable loop wakes promptly — without this, \
             new AC entries advertise only on the 60s backstop tick \
             instead of within the slow-write window",
        );
    Ok(())
}

/// `remove_local_ac_pins` MUST be a strict subset op: it removes only
/// `dispatched_mirror_pins` entries matching the supplied digests, and
/// MUST NOT touch `mirror_blobs` / `mirror_blobs_total_bytes` (which
/// are used by the unrelated CAS server→worker mirror path). A bug
/// here would either over-evict CAS mirror data on AC ack or
/// under-evict AC pins on BIS, both of which corrupt advertisement.
///
/// Mutation step: change `pins.retain` to `pins.clear()` in
/// `remove_local_ac_pins`. The "AC pin entry for d_keep MUST survive"
/// assertion red-fails.
#[nativelink_test]
async fn remove_local_ac_pins_only_touches_pin_index() -> Result<(), Error> {
    let fast_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let slow_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let fast_slow_store = FastSlowStore::new(
        &FastSlowSpec {
    ...
    // CAS mirror entry — populates mirror_blobs AND dispatched_mirror_pins.
    let d_cas = DigestInfo::try_new(VALID_HASH, 16).unwrap();
    fast_slow_store
        .insert_dispatched_mirror_blob("cas", d_cas, Bytes::from(vec![0u8; 16]))
        .expect("CAS mirror insert MUST succeed");

    // AC pin entries — only dispatched_mirror_pins.
    let alt_hash: String = format!("f{}", &VALID_HASH[1..]);
    let d_ack = DigestInfo::try_new(&alt_hash, 92).unwrap();
    let alt2: String = format!("e{}", &VALID_HASH[1..]);
    let d_keep = DigestInfo::try_new(&alt2, 92).unwrap();
    fast_slow_store.insert_local_ac_pin("AC_MAIN_STORE", d_ack);
    fast_slow_store.insert_local_ac_pin("AC_MAIN_STORE", d_keep);

    assert_eq!(fast_slow_store.dispatched_mirror_pin_snapshot().len(), 3);
    assert_eq!(fast_slow_store.mirror_blob_count(), 1);
    let cas_used_before = fast_slow_store.mirror_blobs_used_bytes();
    assert_eq!(cas_used_before, 16);

    // Server acks d_ack only — must NOT touch CAS mirror state, must
    // NOT remove d_keep, must NOT touch d_cas pin entry.
    fast_slow_store.remove_local_ac_pins(&[d_ack]);

    let snap = fast_slow_store.dispatched_mirror_pin_snapshot();
    assert_eq!(
        snap.len(),
        2,
        "remove_local_ac_pins MUST remove exactly one (the d_ack pin); \
         d_cas mirror pin and d_keep AC pin must survive; got {snap:?}"
    );
    assert!(
        snap.iter().any(|(sid, d)| sid.as_ref() == "AC_MAIN_STORE" && *d == d_keep),
        "AC pin entry for d_keep MUST survive a remove_local_ac_pins \
         that ack'd only d_ack — the over-action sibling of the under-action \
         (failing to remove d_ack) is wrongly removing d_keep too; got {snap:?}"
    );
    assert!(
        snap.iter().any(|(sid, d)| sid.as_ref() == "cas" && *d == d_cas),
        "CAS dispatcher pin entry for d_cas MUST survive — \
         remove_local_ac_pins must NOT touch CAS pins; got {snap:?}"
    );
    assert_eq!(
        fast_slow_store.mirror_blob_count(),
        1,
        "remove_local_ac_pins MUST NOT touch mirror_blobs (CAS data) — \
         AC bytes never lived there"
    );
    assert_eq!(
        fast_slow_store.mirror_blobs_used_bytes(),
        cas_used_before,
        "remove_local_ac_pins MUST NOT decrement mirror_blobs_total_bytes — \
         that counter belongs to the CAS mirror map"
    );
