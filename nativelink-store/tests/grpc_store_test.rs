...
use nativelink_proto::build::bazel::remote::execution::v2::{
    ...
    SplitBlobRequest, SplitBlobResponse, UpdateActionResultRequest, batch_update_blobs_response,
    chunking_function, digest_function,
};
...
struct FakeStreamServer {
    write_requests: Arc<AsyncMutex<Vec<WriteRequest>>>,
    read_requests: Arc<AsyncMutex<Vec<ReadRequestHolder>>>,
}

impl FakeStreamServer {
    fn new() -> Self {
        Self {
            write_requests: Arc::new(AsyncMutex::new(vec![])),
            read_requests: Arc::new(AsyncMutex::new(vec![])),
        }
    }
}
...
// ---------------------------------------------------------------------------
// SplitBlob/SpliceBlob (#2497) native forwarding through a grpc proxy store.
// ---------------------------------------------------------------------------

#[derive(Debug, Clone)]
struct FakeCasServer {
    split_requests: Arc<AsyncMutex<Vec<SplitBlobRequest>>>,
    splice_requests: Arc<AsyncMutex<Vec<SpliceBlobRequest>>>,
}

impl FakeCasServer {
    fn new() -> Self {
        Self {
            split_requests: Arc::new(AsyncMutex::new(vec![])),
            splice_requests: Arc::new(AsyncMutex::new(vec![])),
        }
    }
}

type FakeCasGetTreeStream =
    Pin<Box<dyn Stream<Item = Result<GetTreeResponse, Status>> + Send + 'static>>;

#[tonic::async_trait]
impl ContentAddressableStorage for FakeCasServer {
    type GetTreeStream = FakeCasGetTreeStream;
