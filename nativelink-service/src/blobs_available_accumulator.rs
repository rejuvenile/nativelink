...
struct HeaderScalars {
    ...
    // (decayed p95 ms), carried from chunk 0. LOGGED (`tag =
    ...
    fn merge(&mut self, chunk: BlobsAvailableChunk) -> Result<bool, &'static str> {
        ...
        if chunk.sequence == 0 {
            ...
            self.header_scalars = Some(HeaderScalars {
                ...
                construct_latency_ms_p95: chunk.construct_latency_ms_p95,
            });
        }
        ...
        match merge_result {
            ...
            Ok(is_terminal) => {
                ...
                if let Some(headers) = &removed.header_scalars {
                    ...
                    body.construct_latency_ms_p95 = headers.construct_latency_ms_p95;
                }
...
mod tests {
    ...
    /// (#obs-tuning) `construct_latency_ms_p95` rides chunk 0 and the
    ...
    /// dropping it here would make the scheduler's `T_SETUP`-tuning log read a
    /// constant 0 in prod.
    ///
    /// Mutation step (CLAUDE.md TDD #5): comment out the
    /// `body.construct_latency_ms_p95 = headers.construct_latency_ms_p95;`
    ...
    fn construct_latency_carried_forward_from_chunk_zero() {
        let acc = BlobsAvailableAccumulator::new();
        let mut c0 = chunk(1, 0, false, 99, vec![bdi(1)]);
        c0.construct_latency_ms_p95 = 4_242;
        ...
        let out = acc.merge_chunk(c1).expect("terminal commits");
        assert_eq!(
            out.construct_latency_ms_p95, 4_242,
            "#obs-tuning: chunk-0 construct_latency_ms_p95 was lost in chunked \
