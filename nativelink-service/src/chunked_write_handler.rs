...
/// #212 v4.5: `CasExtensions` trait impl on the bare handler type. The
/// v2 RPC requires `Arc<Self>` for per-session task spawning, so the
/// production trait impl lives on the dedicated
/// `ChunkedCasExtensionsAdapter` newtype below; both wire to the same
/// underlying `ChunkedWriteHandler`.
///
/// The legacy v1 `WriteChunked` RPC has been removed — workers upload via
/// `WriteChunkedV2` fleet-wide. `write_chunked` is retained only to satisfy
/// the generated trait (the proto method still exists for wire
/// compatibility) and returns `Code::Unimplemented`.
    ...
    for ChunkedWriteHandler<Fe>
{
    ...
    /// Legacy v1 `WriteChunked` — REMOVED. Workers upload via
    /// `WriteChunkedV2`; the Bazel-facing internal-chunking path uses the
    /// in-process `BazelChunkedDispatcher`. Returns `Code::Unimplemented`.
    async fn write_chunked(
        &self,
        _request: Request<Streaming<WriteChunk>>,
    ) -> Result<Response<WriteChunkedResponse>, Status> {
        Err(Status::unimplemented(
            "WriteChunked (v1) removed; workers upload via WriteChunkedV2",
        ))
    }
...
///
/// The bare-handler trait impl above stays so the in-tree integration
/// tests that pass a bare `ChunkedWriteHandler` keep compiling.
///
    ...
    for ChunkedCasExtensionsAdapter<Fe>
{
    ...
    /// Legacy v1 `WriteChunked` — REMOVED (returns `Code::Unimplemented`).
    /// Workers upload via `WriteChunkedV2`.
    async fn write_chunked(
        &self,
        _request: Request<Streaming<WriteChunk>>,
    ) -> Result<Response<WriteChunkedResponse>, Status> {
