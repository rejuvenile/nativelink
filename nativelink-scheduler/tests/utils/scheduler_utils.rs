...
pub(crate) fn update_eq(
    ...
    match actual_update {
        ...
        update_for_worker::Update::UploadMissingBlobs(actual_update) => {
            match expected_update {
                update_for_worker::Update::UploadMissingBlobs(expected_update) => {
                    expected_update == actual_update
                }
                _ => false,
            }
        ...
        update_for_worker::Update::ChunkedMessage(actual_update) => match expected_update {
            update_for_worker::Update::ChunkedMessage(expected_update) => {
                expected_update == actual_update
            }
            _ => false,
        },
